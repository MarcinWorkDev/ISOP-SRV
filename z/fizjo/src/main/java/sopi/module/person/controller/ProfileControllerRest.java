...
public class ProfileControllerRest {
	...
	public StatusHelper set(@PathVariable Long id, @Valid @RequestBody Profile profile) {
		...
		try {
			...
			return new StatusHelper(true, "Rekord zosta³ zaktualizowany.", profile);
		} catch (Exception e) {
			return new StatusHelper(false, "Wyst¹pi³ b³¹d: " + e.getMessage(), profile);
		}
	}
	...
	public StatusHelper add(@Valid @RequestBody Profile profile) {
		if (profileModel.getProfile(profile.getPesel()) != null){
			return new StatusHelper(false, "W systemie istnieje ju¿ profil z podanym numerem PESEL.",profile);
		} else {
			try {
				profile.setCreatedAt(new DateTime(new Date()));
				profile.setCreatedBy("ADMIN_DEV");
				profileModel.saveNew(profile);
				return new StatusHelper(true, "Rekord zosta³ dodany.", profile);
			} catch (Exception e) {
				return new StatusHelper(false, "Wyst¹pi³ b³¹d: " + e.getMessage(), profile);
			}
		}
	...
	public StatusHelper delete(@PathVariable Long id){
		try {
			...
			return new StatusHelper(true,"Profil zosta³ usuniêty", id);
		} catch (Exception e) {
